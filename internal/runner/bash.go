...
func runBash(tempDir string, req types.RunRequest) types.RunResponse {
	...
	for _, test := range req.Tests {
		result := runBashTest(tempDir, test)
...
func runBashTest(tempDir string, test types.TestCase) types.TestResult {
	start := time.Now()
	...
	stderr = newCappedBuffer(config.MaxCapturedOutputLen)
	stderr.Write([]byte(processStderr(rawStderr, "bash")))
	logNsjailFile(tempDir, "bash")
